...
process_cluster <- function(cluster_ID, k = K, idx = NA, n_total = NA) {
    ...
    tryCatch({
        ...
        # Reprend la base flanking déjà construite (kmc_db) pour extraire, cette
        # fois, TOUS les reads tumeur au locus (REF + ALT), afin d'estimer une
        # VAF réelle plutôt que de se fier au seul comptage tumour-specific.
        # Le test binomial (H0 = erreur de séquençage, p = P_NULL_SEQ_ERROR)
        # sert de filtre de confiance supplémentaire, indépendant du germline
        # filter : un variant peut être somatique (non-germline) mais rester
        # statistiquement indissociable du bruit s'il n'a que 1-2 reads ALT.
        ...
        # ── Ne garder que les mutations significatives, dès maintenant ─────────
        # On élague ici, avant tout traitement supplémentaire : un cluster dont
        # AUCUN mismatch n'est is_true_mutation ne calcule même pas les métriques
        # d'alignement ci-dessous, et surtout n'est jamais écrit dans contig_fa
        # -> jamais aligné par minimap2 en section 5 (l'étape la plus coûteuse
        # à éviter). is_true_mutation vaut TRUE pour toutes les lignes restantes
        # une fois filtré, donc la colonne n'est plus reportée en sortie.
...
snv_rows <- lapply(seq_len(nrow(sam)), function(i) {
    ...
    lapply(seq_len(nrow(mm)), function(j) {
        ...
        # vaf_df a exactement le meme nombre de lignes que mm (= mm_somatic),
        # dans le meme ordre -> indexation directe par j, pas de merge requis.
        vaf_row <- x$vaf_df[j, ]
